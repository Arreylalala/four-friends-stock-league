...
只修改仓库根目录 `scoreboard.json`，提交到 `main` 后自动测试、构建、发布。每人金额统一填写为 `{"stock": 100, "fund": -30}`，分别表示股票、基金盈亏，单位为元。没有持仓或无盈亏明确填 `0`，尚未出结果填 `null`；两项必填，不再支持单个数字。任意一项为 `null`，该人显示待结算；四人两项全部为数字才结算当天。页面金额、排名、走势图、每日胜负和连胜均按股票加基金的合计计算，不展示分项。日期不可重复，收益率单位为百分数。
